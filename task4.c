...
int main()
{
    ...
    for (i = 0; i < Q; i++)
    {
        ...
        from_index = getNodeindex(str_from);
        to_index = getNodeindex(str_to);
        if (from_index != -1 && to_index != -1)
        {
            resetNodeStatus();
            tale = 0;
            searchRoute(str_from, str_to);
        }
        else
        {
            printf("NA\n");
...
void searchRoute(char *from, char *to)
{
    ...
    push(nodes[from_index].ID, nodes[from_index].dist);
    ...
    while (tale != 0)
    {
        ...
        for (i = 0; i < edgenumber; i++)
        {
            ...
            else if (strcmp(from, edges[i].node[1]) == 0)
            {
                flag = 1;
            ...
            if (flag == 1)
            {
                ...
                if (nodes[index].dist > nodes[from_index].dist + edges[i].cost)
                {
                    ...
                    push(nodes[index].ID, nodes[index].dist);
                }
            }
        }
    }
...
// return: number of nodes array element
// example
//imput "C1" -> return num (satisfy node[num].ID == "C1")
...
void push(char *nodeid, float dist)
{
    strcpy(qd[tale].ID, nodeid);
    qd[tale].dist = dist;
    //printf("qd[%d] %s, %f\n", tale, qd[tale].ID, qd[tale].dist);
    tale++;
}
...
void showRoute(char *goal)
{
    int index = getNodeindex(goal);
